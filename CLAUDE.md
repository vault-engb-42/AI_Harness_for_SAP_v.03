...
**The only external substrate is the MCP-ADT server** (ABAP Developer Tools: 17 `aws_abap_cb_*` tools), reached through the local bridge in `mcp-adt-bridge/`. There is **no other backend** — no code graph, no separate analyser, no gateway. Grounding, quality, and delivery all ride the ADT tools. This file is the cached prompt-prefix; keep it small and stable (see P7).
...
Built: this spine, `plugin.json`, the MCP-ADT bridge (`mcp-adt-bridge/`), `.mcp.json`. The agent roster, skills/lanes, hooks, `settings.json`, templates, and `model-tier.js` are being authored against this contract — see `README.md` for what is live vs planned.
