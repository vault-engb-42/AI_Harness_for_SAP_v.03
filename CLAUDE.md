...
- **P2 — Released-API-only, grounded via ADT.** Before code is written, every API/table/CDS the agent proposes is checked with `aws_abap_cb_get_migration_analysis` and confirmed by ATC (variant below). "Released" is a *family* of release contracts, not one grade — ground each usage against the *right* one: **C0 (Extend)** for an extension seam (BAdI/RAP/CDS-extend), **C1 (Use System-Internally)** for an on-stack consumption, **C2 (Use as Remote API)** for OData/remote exposure (C3 config, C4 AMDP). No unreleased-API code ships.
...
- **P5 — Non-prod-only writes, fail-closed.** The 5 ADT write tools (`create_object`, `update_source`, `activate_object`, `activate_objects_batch`, `create_or_update_test_class`) are **disabled by default** at the bridge (`HARNESS_ADT_ALLOW_WRITE` unset ⇒ blocked). They are enabled only against a DEV connection. Promotion to QAS/PRD is human-released transport — never a direct write. No PRD connection is ever registered.
- **P6 — ATC is a gate, not advice.** Every changed object runs `aws_abap_cb_run_atc_check` with variant **`ABAP_CLEAN_CORE_DEVELOPMENT`**, priority-1 **and priority-2 zero** (SAP's transport-blocking config blocks both; priority-3 = notify only), before "done"; `aws_abap_cb_run_unit_tests` must be green.
...
The harness produces **activated objects in DEV + a proof bundle** (`specs/reviews/sap-verdict.json`: activation log, ATC findings with priorities, ABAP Unit results, Clean-Core level, invariant diff). It releases nothing into the transport chain. A human reads the proof and releases the transport DEV→QAS→PRD.

## Build status

Built: this spine, `plugin.json`, the MCP-ADT bridge (`mcp-adt-bridge/`), the **standalone analyser** (`analyser/` — `@abaplint/core` CPG + rule packs at full TALOS parity, its own `abap-analyser` MCP), the **greenfield** grounding + ABAP-Cloud linter (`greenfield/` — released-API grounding + a 59-rule parser-based Clean-Core linter, its own `greenfield` MCP, wired into the design/implement/validate lanes), the **moderniser** (`moderniser/` — the built offline modernisation engine: consumes `analyser-findings.json`, freezes a content-hashed plan, and drives each object GROUND→TRANSFORM→SELF_CHECK→offline-VERDICT; the gap-2b before/after extractor + offline verdict arc are built and offline-operational) and the ported MCP-ADT sidecar (`sap-adt-sidecar/`), `.mcp.json`, the agent roster, and the skills/lanes. The offline pipeline (analyser → moderniser → greenfield/oracle grounding, GF-1..GF-3) is live and engine-direct; only the live-SAP round-trip (GF-4, the moderniser's `--live` DEV push→activate→gate) awaits DEV credentials — see `README.md` for what is live vs planned.
