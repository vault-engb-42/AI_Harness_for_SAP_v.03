...
export function graphTab(doc) {
  ...
  // Node-link layouts hairball past ~40 objects; default big apps to the matrix.
  const objectCount = new Set(all.map((n) => n.object)).size;
  const bigApp = objectCount > 40;
  return `<h2>Dependency Graph — ${all.length} nodes / ${totalEdges} edges</h2>
<p class="muted">${note}Node size = importance (PageRank). Teal = customer · grey = SAP · amber = vendor. The <b>Matrix</b> view scales past a few dozen objects where the node graph tangles.</p>
<div class="gbar"><b>View</b> <button type="button" onclick="gLayout('matrix')">Matrix</button> <button type="button" onclick="gLayout('force')">Force</button> <button type="button" onclick="gLayout('layered')">Layered</button> <span class="sep">·</span> <span class="gzoom"><b>Zoom</b> <button type="button" onclick="gZoom(1.25)">+</button> <button type="button" onclick="gZoom(0.8)">−</button> <button type="button" onclick="gFit()">Fit</button> <button type="button" onclick="gReset()">Reset</button></span></div>
<div class="gwrap" id="gsvgwrap"${bigApp ? ' style="display:none"' : ""}><svg id="gsvg" viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet"><g id="gv">${lines}${dots}</g></svg></div>
<div id="gmatrix"${bigApp ? "" : ' style="display:none"'}>${dependencyMatrix(doc)}</div>`;
}

const MATRIX_CAP = 120; // objects; beyond this the grid caps to the most important
/**
 * Dependency Structure Matrix (DSM): an object×object grid, cell filled = row
 * depends on column. Objects are ordered by topological wave (dependencies first)
 * then importance, so clean layered dependencies fall BELOW the diagonal and a
 * filled cell ABOVE the diagonal is a circular dependency (red). Scales to hundreds
 * of objects as a plain grid — no layout physics, deterministic, the readable answer
 * where a node-link graph hairballs.
 */
function dependencyMatrix(doc) {
  const { objects, adj, kindOf, capped, total } = matrixModel(doc);
  if (!objects.length) return `<p class="muted"><i>No inter-object dependencies to chart.</i></p>`;
  const idx = new Map(objects.map((o, i) => [o, i]));
  let cycleEdges = 0;
  const rows = objects.map((o, i) => {
    const deps = adj.get(o) ?? new Set();
    const cells = objects.map((t, j) => {
      if (i === j) return `<td class="mx-diag"></td>`;
      if (!deps.has(t)) return "<td></td>";
      const cycle = j > i; // above the diagonal = depends on a later object = a cycle
      if (cycle) cycleEdges += 1;
      return `<td class="mx-dep${cycle ? " mx-cycle" : ""}" title="${esc(o)} → ${esc(t)}${cycle ? " (cycle)" : ""}"></td>`;
    }).join("");
    return `<tr><th class="mx-row" title="${esc(kindOf.get(o) ?? "")}">${i + 1}. ${esc(o)}</th>${cells}</tr>`;
  }).join("");
  const header = `<tr><th class="mx-corner"></th>${objects.map((o, j) => `<th class="mx-col" title="${esc(o)}">${j + 1}</th>`).join("")}</tr>`;
  const capNote = capped ? `<b>Showing the top ${objects.length} of ${total} objects by importance.</b> ` : "";
  return `<p class="muted small">${capNote}${objects.length} objects · rows depend on columns · <span class="mx-key mx-dep"></span> dependency (below diagonal = clean layering) · <span class="mx-key mx-cycle"></span> circular dependency (${cycleEdges}). Hover a cell for the pair.</p>
<div class="mxwrap"><table class="mx"><thead>${header}</thead><tbody>${rows}</tbody></table></div>`;
}

/** Object-level adjacency (methods collapsed into their object) + a deterministic topological order. */
function matrixModel(doc) {
  ...
  for (const n of nodes) {
    ...
    if (!kindOf.has(n.object)) kindOf.set(n.object, n.kind);
  }
  const adj = new Map();
  ...
  const wave = topoWaves(objSet, adj);
  let objects = [...objSet].sort((a, b) => (wave.get(a) ?? 0) - (wave.get(b) ?? 0) || (rankOf.get(b) ?? 0) - (rankOf.get(a) ?? 0) || (a < b ? -1 : 1));
  const total = objects.length;
  const capped = total > MATRIX_CAP;
  if (capped) {
    const keep = new Set([...objects].sort((a, b) => (rankOf.get(b) ?? 0) - (rankOf.get(a) ?? 0)).slice(0, MATRIX_CAP));
    objects = objects.filter((o) => keep.has(o)); // preserve topological order
  }
  return { objects, adj, kindOf, capped, total };
}

/** Topological depth from leaves (deps first); a cycle back-edge degrades to 0, not a loop. */
function topoWaves(objSet, adj) {
  const wave = new Map();
  ...
  const compute = (o) => {
    ...
    visiting.add(o);
    let w = 0;
    for (const d of adj.get(o) ?? []) if (objSet.has(d)) w = Math.max(w, compute(d) + 1);
    visiting.delete(o);
    wave.set(o, w);
    return w;
  };
  for (const o of objSet) compute(o);
  return wave;
}
