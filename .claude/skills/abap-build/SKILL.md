...
- **Treating syntax-green as a gate.** `/abap-implement` runs `check_syntax` only and renders no verdict. A group is not done until `/abap-validate` writes a PASS `sap-verdict.json`. The generator writes; the evaluator grades — never let the implement phase declare itself finished.
