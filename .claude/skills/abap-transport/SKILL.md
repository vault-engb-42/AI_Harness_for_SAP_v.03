...
3. **`security-verdict.json` `pass` is `true`** — all four P4 invariants `ok` (`AUTHORITY-CHECK` not weakened, `COMMIT WORK` not suppressed, the RAP save `COMMIT ENTITIES` not suppressed, `SY-SUBRC` check not dropped) and `baseline_established` true. A P4 invariant regression is un-deliverable regardless of ATC/unit greenness. Missing file ⇒ fail-closed ⇒ STOP.
...
- The evidence roll-up: verdict, activation log, ATC (variant + priority-1 empty + priority-2 empty + priority-3 recorded), ABAP Unit (failed empty + coverage vs baseline), `clean_core_level:A`, `invariant_diff` (all four false), and the clean-core/security/diff verdict pointers with `pass:true`.
...
- **WARN is deliverable, BLOCK is not.** A `WARN` sap-verdict (priority-3 ATC within the accepted ratchet, or a coverage note) is release-ready with the WARNs recorded for the human to weigh. A `BLOCK`, a non-empty `atc.priority1`, a non-empty `atc.priority2`, or a non-empty `abap_unit.failed` is never assembled into a ready transport.
