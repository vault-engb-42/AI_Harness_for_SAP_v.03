...
5. **Loop** to step 2 until an exit condition:
   ...
   - **Offline (the default) — the run ends earlier BY DESIGN:** when `next` returns `[]` and every dispatched node sits at `SYNTAX_OK`, the offline pass is done. Do NOT invent checkpoint evidence and do NOT walk nodes past `SYNTAX_OK` — the DEV conjuncts (push/activate/ATC/Unit/parity confirmation) are `BLOCKED-ON-DEV-CREDS`, so no node can reach `GREEN`, and dependents whose closure never greens stay `PENDING` (report them as `BLOCKED-ON-DEV-CREDS(closure)`). This is the honest offline ceiling: generation + lint for the first ready frontier, provisional everything else.
6. **Proof bundle.** Assemble `specs/reviews/sap-verdict.json` from the run log + per-node state: online — activation log, ATC findings with priorities, ABAP Unit results, Clean-Core level, invariant diff, parity scores; offline — per node: generation status, lint result, `verdict: provisional`, `blocked_on: DEV-CREDS`, and un-dispatched nodes marked `BLOCKED-ON-DEV-CREDS(closure)`. Hand to `/abap-transport` for assembly — the human releases.
