...
**The proof bundle.** Assemble `specs/reviews/sap-verdict.json` from the run log + per-node state + sweep ledger: live — activation log, ATC findings with priorities, ABAP Unit results, Clean-Core level, invariant diff, parity scores; offline — per node: `status: gated-provisional` (walked the loop to `SYNTAX_OK`) or `draft-swept` (sweep only), generation + lint result, `verdict: provisional`, `blocked_on: DEV-CREDS`. Hand to `/abap-transport` for assembly — the human releases.
