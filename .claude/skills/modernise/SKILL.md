...
1. **Plan.** `node moderniser/src/cli.js plan <findings> --bundle <dir> [--team-size N]` → `{run_id, plan_hash, nodes, waves}`. **Offline, `--bundle` (the SAME abapGit directory the analyser scanned) is MANDATORY** — it runs the Stage-1 dynamic scan (`CALL FUNCTION <var>`, BAdIs, dynamic DML, `PERFORM … ON COMMIT`, …): unresolvable constructs SEAL their node behind the human seam gate and resolvable late-calls close hidden cycles BEFORE Tarjan, all baked into the frozen `plan_hash`. Omitting it plans an UNSEALED graph — under-approximation, the direction L5 forbids; do that only when no source bundle exists (live-pull mode). If the run already exists the CLI refuses — use `resume <run_id>` (never `--force` a live run without operator approval). Then `git add` the written plan/state paths (the CLI fsyncs; the shell owns git). Present the wave table to the operator ONLY if `--breakpoint` or an escalation asks for it — otherwise proceed (human-by-exception, L2).
...
4. **Verdict (online).** With the node at `GATED`, run `... verdict <run_id> <sig> --checkpoint cp.json --evidence ev.json --record` — the checkpoint/evidence JSONs are the **evaluator's** files, never authored by this orchestrator (GAN). The CLI composes ratchet gate + node verdict with the signed WARN delta and records the result; `--record` moves the baselines ONLY on green. On a RED verdict with retry budget left (cycle < 3), retry via `... progress <run_id> <sig> GENERATED` — the FSM counts the cycle and voids the stale verdict — and re-walk; only at the ceiling (or on a non-retryable reason) report `... outcome <run_id> <sig> BLOCK --reason <r>`. On green: `... outcome <run_id> <sig> GREEN` (refused without the recorded green verdict) and `git add` the state/baseline paths.
5. **Loop** to step 2 until an exit condition:
   ...
   - **Offline (the default) — the gated pass ends at its ceiling, then the DRAFT SWEEP runs (§6.5, ratified 2026-07-11):** when `next` returns `[]` and **no dispatched node can advance** — every one sits at `SYNTAX_OK` or in a non-advancing human/terminal state (`BLOCK` from a syntax ceiling, `PARK`, `NEEDS_MANUAL_SEAM`) — the gated pass is done (L12: a machine-BLOCK does not wedge the exit; its starved PENDING dependents are exactly what 5b sweeps). Do NOT invent checkpoint evidence and do NOT walk nodes past `SYNTAX_OK` — no node can reach `GREEN` offline. Then sweep the rest (step 5b).
