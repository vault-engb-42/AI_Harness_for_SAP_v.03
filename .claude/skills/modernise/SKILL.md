...
2. **Gate 2 — architecture.** `node moderniser/src/cli.js arch <R> <findings> [--model <judge-model>]` → reasons a `target_shape` per `re_architect`/`rebuild` node, proves the app blueprint consistent, freezes one COARSE Architecture Contract per RESOLVED node, writes `specs/runs/<R>/architecture-manifest.json`, and raises one `ARCH_REVIEW` per resolved node. Its `.pending[]` rows are the nodes the deterministic matcher could not settle alone — each carries `fact` (the P8 fact stream) and the offered `candidates`.
   ...
   - **Independent review.** Spawn `abap-arch-reviewer` (fresh context, the GAN counter-party to the judge) per resolved row; attach its verdict to the packet the human reads.
   - **Ratify.** Surface each `ARCH_REVIEW` packet; a named human decides `... decide <R> <esc_id> <approve|refine:<notes>|reject> --by <name>`. Only `approve` ratifies (binding the contract hash); `refine`/`reject` VOID any prior ratification. A later `arch` re-run PRESERVES an unchanged ratified contract and voids a changed one — re-ratification is required only for what actually changed.
