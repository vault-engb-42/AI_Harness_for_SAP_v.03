...
- **Treating syntax-green as done:** This lane does not gate. Run `/abap-validate` on the group after this skill completes — `abap-evaluator` pushes the UNCHANGED source to DEV, activates, runs ATC (`ABAP_CLEAN_CORE_DEVELOPMENT`, priority-1 zero) and ABAP Unit, and writes `specs/reviews/sap-verdict.json`. Until that lane passes, the group is not merge-ready.
