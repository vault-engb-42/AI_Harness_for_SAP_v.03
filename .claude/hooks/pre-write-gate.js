...
if (isAbapSource(filePath)) {
  ...
  if (inv.length) {
    block(`immutable invariant regression (${inv.map((i) => i.type).join(", ")}). Authorization — classic AUTHORITY-CHECK, the RAP BDEF \`authorization master ( … )\` + its GET_*_AUTHORIZATIONS handlers, and DCL GRANT (P4a) — plus COMMIT ENTITIES (RAP save; or COMMIT WORK in classic code) and SY-SUBRC are un-loosenable (P4) — restore the guard.`);
  }
}
