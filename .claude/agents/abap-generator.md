...
- **@UI Fiori-Elements readiness (P2 / UI surface).** A consumption view (`ZC_*`) exposed to a UI OData service MUST carry the minimum `@UI` set so a Fiori Elements **List Report + Object Page** render with NO hand-written UI: `@UI.headerInfo` (typeName / typeNamePlural / title), ≥1 positioned `@UI.lineItem` (list columns), ≥1 `@UI.selectionField` (filter bar), and Object-Page content (`@UI.facet` + `@UI.identification`); `@UI.dataPoint` only where a KPI is shown. Put the `@UI` **inline** on the ZC_ projection, or externalise it in a **DDLX metadata extension** (`metadata-extension.template.abap`) when the UI evolves independently — the FE-readiness gate accepts either location but blocks a projection that carries `@UI` yet omits part of the min-set (`gf-x-ui-fe-readiness`).
- **Fiori Elements app project (G12 / S1, UI surface).** When the group builds a UI-facing OData service (an `@UI` ZC_ projection + SRVD + an OData V4 **UI** SRVB), also author the deployable **Fiori Elements app project** from `.claude/templates/fiori-elements-app.template.json`: a `manifest.json` whose OData `dataSource` resolves to the **published** SRVB service (G10), `sap.fe.templates` **List Report + Object Page** floorplans (no hand-written view/controller code), and a main `entitySet` that IS the ZC_ projection entity (never the ZI_ interface view). The descriptor-validation gate (`mcp__greenfield__validate_fe_descriptor`, wired into `/abap-validate`) blocks a manifest missing the dataSource, the FE floorplans, or the projection entitySet. A Web-API-only (headless) service gets no app project.
