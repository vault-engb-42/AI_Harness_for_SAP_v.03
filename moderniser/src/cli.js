 ...
 *   arch-verdict <run_id> <findings.json> <sig> --shape <target_shape> --by <judge> --rationale <why> --confidence <high|medium|low> [--model m] [--prompt-hash h] [--shared-json f]   (the judge's write seam: record a selection for an await_arch node; the next `arch` resolves it from cache. --rationale/--confidence are the judge's own words + grade and ride the ratification row — the human ratifies by exception and needs WHY, not only who. --model/--prompt-hash MUST match the outstanding request. --shared-json carries the judge's app-level grouping {services|projections|fiori_apps: [{id, members}]}, unioned by label across nodes)
